...
function run(args) {
  return spawn(process.execPath, args, { cwd: process.cwd(), env: environment, stdio: "inherit", windowsHide: true });
}
...
try {
  ...
  await Promise.all([ready("http://127.0.0.1:18081", fixture), ready("http://localhost:3100/admin/login", next)]);
